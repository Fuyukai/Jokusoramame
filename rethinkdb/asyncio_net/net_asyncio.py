...
from rethinkdb.net import Query, Response, Cursor, maybe_profile
...
class AsyncioCursor(Cursor):
    ...
    async def __anext__(self):
        can_fetch = await self.fetch_next()
        if not can_fetch:
...
class ConnectionInstance(object):
    ...
    def connect(self, timeout):
        try:
            ...
            self._streamreader, self._streamwriter = yield from asyncio.open_connection(self._parent.host,
                                                                                        self._parent.port,
                                                                                        loop=self._io_loop,
                                                                                        ssl=ssl_context)
